...
class EleicaoDAO {
    ...
    public function excluir(int $id): bool {
        try {
            ...
            error_log("Erro ao excluir eleição: " . $e->getMessage());
            return false;
        }
    }

    public function contarVotos(int $idEleicao): int {
        try {
            $sql = "SELECT COUNT(*) as total FROM voto WHERE eleicao_fk = :id";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([':id' => $idEleicao]);
