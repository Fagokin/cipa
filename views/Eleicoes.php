...
if (isset($_GET['msg'])) {
    if ($_GET['msg'] === 'sucesso') {
        $mensagem = '<div class="alert alert-success alert-dismissible fade show mt-3">
                        <strong>Sucesso!</strong> Eleição excluída com sucesso!
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                     </div>';
    } elseif ($_GET['msg'] === 'erro') {
        $mensagem = '<div class="alert alert-danger alert-dismissible fade show mt-3">
                        <strong>Erro!</strong> Não foi possível excluir a eleição.
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                     </div>';
    }
}
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="GerenciarEleicao.php?id=<?= $e['id_eleicao'] ?>" class="btn btn-outline-primary btn-sm">
                                        <i class="fas fa-users"></i> Gerenciar
                                    </a>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-info"><?= $eleicaoDAO->contarVotos($e['id_eleicao']) ?> votos</span>
                                </td>
                                <td class="text-center">
                                    <div class="btn-group" role="group">
                                        <a href="ValidarDados.php?id=<?= $e['id_eleicao'] ?>" class="btn btn-info btn-sm" title="Link de votação">
